...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!routeKey) return new Response(JSON.stringify({ error: "routeKey required" }), { status: 400, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
    ...
    if (count === 0) return new Response(JSON.stringify({ error: "Quote not found." }), { status: 404, headers: { ...getCorsHeaders(req), "Content-Type": "application/json" } });
