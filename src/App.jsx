...
import { pageWhatsAppLink, waLink } from "./utils/links";
...
import { trackEmailClick, trackQuoteSubmit } from "./utils/analytics";
  ...
  return(
    <div style={{marginTop:12,borderRadius:12,border:`1px solid ${B.primary}22`,overflow:"hidden",maxWidth:"100%"}}>
      <div style={{background:B.primary,padding:"12px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div style={{color:"#fff",fontWeight:700,fontSize:14}}>✓ Freight Rate Details — {EQ_L[selectedEq]||selectedEq}</div>
      ...
      <div style={{padding:"14px 18px",background:"#f8faff",borderTop:`1px solid ${B.g3}22`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,boxSizing:"border-box"}}>
        <div style={{fontSize:11,color:B.g5,fontStyle:"italic",flex:1,minWidth:200}}>
          Rate details are subject to space availability, carrier confirmation, and applicable exchange rates at time of booking.
        </div>
...
const handleSubmit=async(ev)=>{ev.preventDefault();if(sending||!validate())return;setSending(true);setSendErr("");const polName=POL.find(p=>p.c===f.pol)?.n||f.pol;const podName=ALL_POD.find(p=>p.c===f.pod)?.n||f.pod;try{await submitQuoteAPI({pol:`${polName} (${f.pol})`,pod:`${podName} (${f.pod})`,equipment:`${EQ_L[f.eq]||f.eq} (${f.eq})`,containers:parseInt(f.vol)||1,cargo:f.cargo,notes:f.msg||"",name:f.name,company:f.company||"",email:f.email,phone:f.phone||"",rateFound:false},"anon");trackQuoteSubmit({origin:polName,destination:podName,containerType:f.eq});setDone(true);}catch(err){setSendErr("Submission failed. Please email quotes@sattvaglobal.in or call +91 9136 121 123 directly.");}finally{setSending(false);}};
...
return(<div style={{paddingTop:68}}><Helmet><title>Get an Export Freight Quote from India | Sattva Global Logistics</title><meta name="description" content="Request an export freight quote from India for Gulf, Red Sea and Africa shipments." /><meta name="robots" content="noindex, nofollow"/><link rel="canonical" href="https://www.sattvaglobal.in/quote"/></Helmet>
...
<form onSubmit={handleSubmit} noValidate><div style={{...st.cd,padding:m?20:36}}><h3 style={{...st.h3,marginBottom:6}}>Shipment Details</h3><p style={{fontSize:13,color:B.g5,marginBottom:24}}>Route, cargo, and equipment.</p>
...
<div style={{display:"grid",gridTemplateColumns:m?"1fr":"1fr 1fr",gap:16,marginBottom:16}}><CargoCombo label="Cargo Type *" value={f.cargo} onChange={v=>{up("cargo",v);setErrs(p=>({...p,cargo:""}));}} error={errs.cargo} placeholder="Search cargo or HS code…" history={[]}/><div><label style={st.lb}>Equipment *</label><select aria-invalid={!!errs.eq} aria-describedby={errs.eq?"qp-eq-err":undefined} style={{...st.inp,borderColor:errs.eq?B.red:undefined}} value={f.eq} onChange={e=>{up("eq",e.target.value);setErrs(p=>({...p,eq:""}));}}><option value="">Select</option>{EQ.map(e=><option key={e} value={e}>{EQ_L[e]} ({e})</option>)}</select><ErrMsg msg={errs.eq}/></div></div>
...
<div style={{display:"grid",gridTemplateColumns:m?"1fr":"1fr 1fr",gap:16,marginBottom:16}}><div><label style={st.lb}>Name *</label><input aria-invalid={!!errs.name} aria-describedby={errs.name?"qp-name-err":undefined} style={{...st.inp,borderColor:errs.name?B.red:undefined}} type="text" value={f.name} onChange={e=>{up("name",e.target.value);setErrs(p=>({...p,name:""}));}} placeholder="Full name"/><ErrMsg id="qp-name-err" msg={errs.name}/></div><div><label style={st.lb}>Company</label><input style={st.inp} type="text" value={f.company} onChange={e=>up("company",e.target.value)} placeholder="Company name"/></div><div><label style={st.lb}>Email *</label><input aria-invalid={!!errs.email} aria-describedby={errs.email?"qp-email-err":undefined} style={{...st.inp,borderColor:errs.email?B.red:undefined}} type="email" value={f.email} onChange={e=>{up("email",e.target.value);setErrs(p=>({...p,email:""}));}} placeholder="you@company.com"/><ErrMsg id="qp-email-err" msg={errs.email}/></div><div><label style={st.lb}>Phone</label><PhoneField value={f.phone} onChange={v=>up("phone",v)} error={errs.phone||phoneErr} onError={setPhoneErr} st={st}/></div></div>
...
{sendErr&&<div role="alert" style={{marginBottom:16,padding:"12px 16px",borderRadius:8,background:"#fef2f2",border:"1px solid #fecaca",color:B.red,fontSize:13}}>{sendErr}</div>}
<button type="submit" disabled={sending} style={{...st.bp,width:"100%",justifyContent:"center",fontSize:15,padding:"14px 24px",opacity:sending?.7:1}}>{sending?"Submitting…":"Submit Quote Request →"}</button>
<p style={{fontSize:11,color:B.g5,marginTop:10,textAlign:"center"}}>We respond within 4 working hours. Urgent: <a href="tel:+919136121123" style={{color:B.g5}}>+91 9136 121 123</a></p></div></form>
...
const WA_FLOAT=pageWhatsAppLink(loc.pathname);
return(
<a href={WA_FLOAT} target="_blank" rel="noopener noreferrer" title="Chat on WhatsApp"
  className="sg-wa-sticky"
  style={{position:"fixed",bottom:28,right:28,zIndex:999,width:56,height:56,borderRadius:"50%",background:"#25D366",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(37,211,102,.5)",textDecoration:"none",transition:"transform .2s"}}
...
<Routes><Route path="/*" element={<FloatingWhatsApp/>}/></Routes>
</div>
</BrowserRouter>
</HelmetProvider>);}
