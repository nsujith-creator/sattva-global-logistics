...
/* ═══ EMAILJS CONFIG ═══ */
const EJS = {
  serviceId:  "service_19yldqj",
  templateId: "template_6n1jgzr",
  otpTemplateId: "template_hn8h6jg",
  publicKey:  "mEeQ8ZUINXOPAeM8F",
};
...
/* ═══ PACKING TYPES ═══ */
const PACK_TYPES=["Cartons / Cardboard Boxes","Wooden Crates","Steel Crates","Pallets (Wooden)","Pallets (Plastic)","Steel Drums","Plastic Drums","PP Woven Bags","Jumbo Bags (FIBC)","Rolls / Coils","Bundles / Straps","Loose / Bulk","IBC (Intermediate Bulk Container)","Tin / Metal Cans","Shrink Wrapped","Vacuum Packed","Other"];
const OT_FR_EQ=["20OT","40OT","20FR","40FR"];

/* ═══ BRAND — Sattva Brand Palette ═══ */
const B = {
  ...
  g3:"#b8cde8",        /* Blue-tinted border */
  g1:"#f4f6fc",        /* Off-white background */
  w:"#fff",
  green:"#059669",gBg:"#ecfdf5",
  red:"#dc2626",amber:"#d97706",aBg:"#fffbeb",
  navy:"#002147",      /* Dark navy for contrast */
  lightBlue:"#cae8ff", /* Light blue tint */
};
const F="'Poppins',sans-serif", FF="'Quicksand',sans-serif";

/* ═══ PORT DATA ═══ */
const POL=[{c:"INNSA",n:"JNPT / Nhava Sheva"},{c:"INMUN",n:"Mundra"},{c:"INMAA",n:"Chennai"},{c:"INKTP",n:"Kattupalli"},{c:"INPAV",n:"Pipavav"},{c:"INTUT",n:"Tuticorin"},{c:"INKOK",n:"Cochin"}];
const POD_R={
"Gulf & Upper Gulf":[
...
const ALL_POD=Object.values(POD_R).flat();
const EQ=["20GP","40GP","40HC","20RF","40RH","20OT","40OT","20FR","40FR","ISO Tank"];
const EQ_L={"20GP":"20' Standard","40GP":"40' Standard","40HC":"40' High Cube","20RF":"20' Reefer","40RH":"40' Reefer","20OT":"20' Open Top","40OT":"40' Open Top","20FR":"20' Flat Rack","40FR":"40' Flat Rack","ISO Tank":"ISO Tank"};
const CARGO=["General Cargo","Chemicals / Hazardous (IMO)","Textiles & Garments","Pharmaceuticals","Food Products","Engineering Goods","Auto Parts","Electronics","Agricultural Products","Machinery","Other"];
const CARRIERS={
  "Maersk":     {color:"#004B93", short:"MSK", logo:"/carriers/maersk.png"},
  "MSC":        {color:"#1a1a6e", short:"MSC", logo:"/carriers/msc.svg"},
  "CMA CGM":    {color:"#0033A0", short:"CMA", logo:"/carriers/cma-cgm.svg"},
  "Hapag-Lloyd":{color:"#FF6600", short:"HL",  logo:"/carriers/hapag-lloyd.png"},
  "ONE":        {color:"#E91B8F", short:"ONE", logo:"/carriers/one.png"},
  "OOCL":       {color:"#003087", short:"OOC", logo:"/carriers/oocl.svg"},
  "COSCO":      {color:"#C8102E", short:"COS", logo:"/carriers/cosco.png"},
  "PIL":        {color:"#003087", short:"PIL", logo:"/carriers/pil.png"},
  "HMM":        {color:"#003B8E", short:"HMM", logo:"/carriers/hmm.png"},
  "Wan Hai":    {color:"#005BAA", short:"WHL", logo:"/carriers/wanhai.png"},
  "NVOCC":      {color:"#6B7280", short:"NVO", logo:""},
};
...
const pn=c=>{const p=POL.find(x=>x.c===c)||ALL_POD.find(x=>x.c===c);return p?p.n:c;};
...
const PK = "sattva-pin"; // PIN stays local
const SESSION_KEY="sattva-verified-user";
const SESSION_DAYS=30;
function saveSession(user){try{localStorage.setItem(SESSION_KEY,JSON.stringify({user,expiry:Date.now()+SESSION_DAYS*24*60*60*1000}));}catch{}}
function loadSession(){try{const s=localStorage.getItem(SESSION_KEY);if(!s)return null;const{user,expiry}=JSON.parse(s);if(Date.now()>expiry){localStorage.removeItem(SESSION_KEY);return null;}return user;}catch{return null;}}
...
async function lr(){
  ...
  try{
    ...
    const tid=setTimeout(()=>ctrl.abort(),5000); // 5 second timeout
...
async function saveRateAPI(key,entry){
...
async function deleteRateAPI(key){
...
async function logSearchAPI(data){
  if(!SCRIPT_URL||SCRIPT_URL==="PASTE_YOUR_SCRIPT_URL_HERE") return;
  try{
    const ctrl=new AbortController();
...
async function lp(){try{return localStorage.getItem(PK);}catch{return null;}}
async function sp(p){try{localStorage.setItem(PK,p);}catch{}}
...
/* ═══ FREE EMAIL DOMAINS — block personal emails ═══ */
const FREE_DOMAINS=["gmail.com","yahoo.com","yahoo.in","hotmail.com","outlook.com","rediffmail.com","ymail.com","live.com","icloud.com","aol.com","protonmail.com"];
const isFreeEmail=(email)=>{const d=email.split("@")[1]?.toLowerCase();return FREE_DOMAINS.includes(d);};

/* ═══ COUNTRY PHONE DATA ═══ */
const COUNTRIES=[
  {c:"IN",flag:"🇮🇳",name:"India",dial:"+91",digits:10,pattern:/^[6-9]\d{9}$/,hint:"10 digits starting with 6-9"},
  {c:"AE",flag:"🇦🇪",name:"UAE",dial:"+971",digits:9,pattern:/^5\d{8}$|^[2-9]\d{7}$/,hint:"9 digits (mobile: starts with 5)"},
  {c:"SA",flag:"🇸🇦",name:"Saudi Arabia",dial:"+966",digits:9,pattern:/^5\d{8}$|^[1-9]\d{7}$/,hint:"9 digits (mobile: starts with 5)"},
  {c:"OM",flag:"🇴🇲",name:"Oman",dial:"+968",digits:8,pattern:/^\d{8}$/,hint:"8 digits"},
  {c:"QA",flag:"🇶🇦",name:"Qatar",dial:"+974",digits:8,pattern:/^\d{8}$/,hint:"8 digits"},
  {c:"KW",flag:"🇰🇼",name:"Kuwait",dial:"+965",digits:8,pattern:/^\d{8}$/,hint:"8 digits"},
  {c:"BH",flag:"🇧🇭",name:"Bahrain",dial:"+973",digits:8,pattern:/^\d{8}$/,hint:"8 digits"},
  {c:"IQ",flag:"🇮🇶",name:"Iraq",dial:"+964",digits:10,pattern:/^7\d{9}$/,hint:"10 digits starting with 7"},
  {c:"YE",flag:"🇾🇪",name:"Yemen",dial:"+967",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"EG",flag:"🇪🇬",name:"Egypt",dial:"+20",digits:10,pattern:/^1[0-9]\d{8}$/,hint:"10 digits starting with 1"},
  {c:"KE",flag:"🇰🇪",name:"Kenya",dial:"+254",digits:9,pattern:/^[17]\d{8}$/,hint:"9 digits"},
  {c:"TZ",flag:"🇹🇿",name:"Tanzania",dial:"+255",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"NG",flag:"🇳🇬",name:"Nigeria",dial:"+234",digits:10,pattern:/^[789][01]\d{8}$/,hint:"10 digits"},
  {c:"GH",flag:"🇬🇭",name:"Ghana",dial:"+233",digits:9,pattern:/^[235]\d{8}$/,hint:"9 digits"},
  {c:"ZA",flag:"🇿🇦",name:"South Africa",dial:"+27",digits:9,pattern:/^[67-9]\d{8}$/,hint:"9 digits starting with 6-9"},
  {c:"ET",flag:"🇪🇹",name:"Ethiopia",dial:"+251",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"DJ",flag:"🇩🇯",name:"Djibouti",dial:"+253",digits:8,pattern:/^\d{8}$/,hint:"8 digits"},
  {c:"SD",flag:"🇸🇩",name:"Sudan",dial:"+249",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"MZ",flag:"🇲🇿",name:"Mozambique",dial:"+258",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"ZM",flag:"🇿🇲",name:"Zambia",dial:"+260",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"ZW",flag:"🇿🇼",name:"Zimbabwe",dial:"+263",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"CD",flag:"🇨🇩",name:"DR Congo",dial:"+243",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"CM",flag:"🇨🇲",name:"Cameroon",dial:"+237",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"CI",flag:"🇨🇮",name:"Cote d'Ivoire",dial:"+225",digits:10,pattern:/^\d{10}$/,hint:"10 digits"},
  {c:"SN",flag:"🇸🇳",name:"Senegal",dial:"+221",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"UG",flag:"🇺🇬",name:"Uganda",dial:"+256",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"RW",flag:"🇷🇼",name:"Rwanda",dial:"+250",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"TR",flag:"🇹🇷",name:"Turkey",dial:"+90",digits:10,pattern:/^5\d{9}$/,hint:"10 digits starting with 5"},
  {c:"IL",flag:"🇮🇱",name:"Israel",dial:"+972",digits:9,pattern:/^5\d{8}$|^[2-9]\d{7}$/,hint:"9 digits"},
  {c:"JO",flag:"🇯🇴",name:"Jordan",dial:"+962",digits:9,pattern:/^7\d{8}$/,hint:"9 digits starting with 7"},
  {c:"LB",flag:"🇱🇧",name:"Lebanon",dial:"+961",digits:8,pattern:/^\d{7,8}$/,hint:"7-8 digits"},
  {c:"PK",flag:"🇵🇰",name:"Pakistan",dial:"+92",digits:10,pattern:/^3\d{9}$/,hint:"10 digits starting with 3"},
  {c:"BD",flag:"🇧🇩",name:"Bangladesh",dial:"+880",digits:10,pattern:/^1[3-9]\d{8}$/,hint:"10 digits starting with 1"},
  {c:"LK",flag:"🇱🇰",name:"Sri Lanka",dial:"+94",digits:9,pattern:/^7\d{8}$/,hint:"9 digits starting with 7"},
  {c:"NP",flag:"🇳🇵",name:"Nepal",dial:"+977",digits:10,pattern:/^9[678]\d{8}$/,hint:"10 digits starting with 96-98"},
  {c:"CN",flag:"🇨🇳",name:"China",dial:"+86",digits:11,pattern:/^1[3-9]\d{9}$/,hint:"11 digits starting with 1"},
  {c:"HK",flag:"🇭🇰",name:"Hong Kong",dial:"+852",digits:8,pattern:/^[2-9]\d{7}$/,hint:"8 digits"},
  {c:"SG",flag:"🇸🇬",name:"Singapore",dial:"+65",digits:8,pattern:/^[689]\d{7}$/,hint:"8 digits"},
  {c:"MY",flag:"🇲🇾",name:"Malaysia",dial:"+60",digits:9,pattern:/^1\d{8,9}$/,hint:"9-10 digits starting with 1"},
  {c:"TH",flag:"🇹🇭",name:"Thailand",dial:"+66",digits:9,pattern:/^\d{9}$/,hint:"9 digits"},
  {c:"ID",flag:"🇮🇩",name:"Indonesia",dial:"+62",digits:10,pattern:/^8\d{9,11}$/,hint:"10-12 digits starting with 8"},
  {c:"JP",flag:"🇯🇵",name:"Japan",dial:"+81",digits:10,pattern:/^[789]0\d{8}$/,hint:"10 digits"},
  {c:"KR",flag:"🇰🇷",name:"South Korea",dial:"+82",digits:10,pattern:/^1[0-9]\d{7,8}$/,hint:"10 digits"},
  {c:"AU",flag:"🇦🇺",name:"Australia",dial:"+61",digits:9,pattern:/^[4578]\d{8}$/,hint:"9 digits starting with 4-5"},
  {c:"NZ",flag:"🇳🇿",name:"New Zealand",dial:"+64",digits:9,pattern:/^[2-9]\d{7,9}$/,hint:"8-10 digits"},
  {c:"GB",flag:"🇬🇧",name:"UK",dial:"+44",digits:10,pattern:/^[789]\d{9}$/,hint:"10 digits starting with 7"},
  {c:"DE",flag:"🇩🇪",name:"Germany",dial:"+49",digits:10,pattern:/^\d{10,11}$/,hint:"10-11 digits"},
  {c:"FR",flag:"🇫🇷",name:"France",dial:"+33",digits:9,pattern:/^[67]\d{8}$/,hint:"9 digits starting with 6-7"},
  {c:"IT",flag:"🇮🇹",name:"Italy",dial:"+39",digits:10,pattern:/^3\d{9}$/,hint:"10 digits starting with 3"},
  {c:"NL",flag:"🇳🇱",name:"Netherlands",dial:"+31",digits:9,pattern:/^6\d{8}$/,hint:"9 digits starting with 6"},
  {c:"ES",flag:"🇪🇸",name:"Spain",dial:"+34",digits:9,pattern:/^[67]\d{8}$/,hint:"9 digits starting with 6-7"},
  {c:"US",flag:"🇺🇸",name:"USA",dial:"+1",digits:10,pattern:/^\d{10}$/,hint:"10 digits"},
  {c:"CA",flag:"🇨🇦",name:"Canada",dial:"+1",digits:10,pattern:/^\d{10}$/,hint:"10 digits"},
  {c:"BR",flag:"🇧🇷",name:"Brazil",dial:"+55",digits:11,pattern:/^\d{10,11}$/,hint:"10-11 digits"},
  {c:"MX",flag:"🇲🇽",name:"Mexico",dial:"+52",digits:10,pattern:/^\d{10}$/,hint:"10 digits"},
];
...
/* ═══ QUOTE PAGE — with instant rate lookup ═══ */
const WA_NUM="919136121123";
